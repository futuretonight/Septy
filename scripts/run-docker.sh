...
CONTAINER="septy"
IMAGE="futuretonight/septy:latest"
IMG_MIRROR="ghcr.io/futuretonight/septy:latest"
...
if [[ ! $check_container == $CONTAINER ]]; then
	...
	docker create \
		--interactive --tty \
		--volume ${MOUNT_LOCATION}:/septy/auth/ \
		--network host \
		--name "${CONTAINER}" \
		"${IMAGE}"
fi
...
# docker run --rm -ti --network="host" -v ${MOUNT_LOCATION}:/septy/auth/ --name "$CONTAINER" "$IMAGE"
